...
function processInput(input: string) {
  ...
  if (inputIndicator.value === primaryIndicator) {
    ...
    if (scopeDepth.value === 0) {
      throw new Error('Scope Depth should not be zero when more input is expected from use')
    } else if (!history.value[history.value.length - 1].more) {
      throw new Error('More HistoryPromptItems should be at least one or empty')
    }
    for (let i = 0; i < input.length; i++) {
      if (input.charAt(i) === '{') {
        scopeDepth.value++
