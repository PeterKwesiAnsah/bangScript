...
size_t internString(Table *strings, Token token, const char *src) {
  ...
  BsObjString lookupKey = {.value = (char *)src + token.start,
                           .len = token.len,
                           .hash = hashString(src + token.start, token.len)};
  ...
  if (found) {
    ...
    BsObjStringFromSource *newObj =
        (BsObjStringFromSource *)malloc(sizeof(BsObjStringFromSource));
