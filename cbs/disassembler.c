...
extern Frame frame;
static const char *opcodeNames[] = {"OP_CONSTANT_ZER0",
                                    "OP_CONSTANT",
                                    "OP_CONSTANT_LONG",
                                    "OP_ADD",
                                    "OP_SUB",
...
DisassemblerStatus disassembleInstruction(uint8_t *ip, uint8_t *start) {
  ...
  case OP_LOCALVAR_GET: {
    ...
    printf("stackSlot[%3d] = ", stackIndex);
    Local var = frame.compiler->locals[stackIndex];
    printf("'%.*s'", var.name.len, (char *)frame.src + var.name.start);
    break;
  }
