...
typedef enum { TYPE_NIL, TYPE_BOOL, TYPE_NUMBER, TYPE_OBJ } BsType;

typedef enum { OBJ_TYPE_STRING_SOURCE, OBJ_TYPE_STRING_ALLOC } BsObjType;
...
#define C_DOUBLE_TO_BS_NUMBER(cdouble)                                         \
  ((Value){.value = {.num = cdouble}, .type = TYPE_NUMBER})
#define BS_NUMBER_TO_C_DOUBLE(bsnumber) (bsnumber.value.num)

#define CREATE_BS_OBJ(objPointer)                                              \
  ((Value){.value = {.obj = (BsObj *)objPointer}, .type = TYPE_OBJ})

#define C_BOOL_TO_BS_BOOLEAN(cbool)                                            \
  ((Value){.value = {.boolean = cbool}, .type = TYPE_BOOL})
